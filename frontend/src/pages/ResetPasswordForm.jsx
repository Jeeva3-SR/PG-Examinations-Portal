...
const ResetPasswordForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(apiError.response?.data?.error || 'Failed to reset password. The link may have expired.');
    } finally {
      setLoading(false);
    }
  };
        ...
        {message ? (
          <div className="space-y-6 animate-fadeIn duration-200">
            ...
            <button
              ...
              className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-xl py-3 transition-all duration-200"
            >
              Sign In Now
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-5">
              ...
              <div>
                <label htmlFor="confirmPassword" className="block text-xs font-semibold uppercase tracking-wider text-slate-600 mb-2">
                  Confirm Password
                </label>
                <input
                  id="confirmPassword"
